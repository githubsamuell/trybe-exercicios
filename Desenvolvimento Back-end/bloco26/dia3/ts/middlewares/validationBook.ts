...
function validationBook(req: Request, res: Response, next: NextFunction) {
    ...
    if(!valid) {
        return res.status(StatusCode.BAD_REQUEST).send(`O campo ${property} é obrigatorio.`)
    }

    [valid, property] = validateValues(book);

    if(!valid) {
        return res.status(StatusCode.BAD_REQUEST).send(`O campo ${property} nao pode ser vazio`)
    }
